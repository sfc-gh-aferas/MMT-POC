    ...
    if actual_category in ["Daily", "Weekly"]:
        ...
        for (year, month), weeks in next_weeks.items():
            for week_idx, week in enumerate(weeks):
                ...
                for dim_x in range(initial_dim_x, 0, -1):
                    try:
                        tasks = [(i, s, dim_x, simulation_period_days) for i, s in enumerate(feat_sc_next_days)]
                        with concurrent.futures.ProcessPoolExecutor() as executor:
                            results = list(executor.map(_kalman_fit_predict, tasks))
                        ...
                        if dim_x == 1:
                            tasks = [(i, s, simulation_period_days) for i, s in enumerate(feat_sc_next_days)]
                            with concurrent.futures.ProcessPoolExecutor() as executor:
                                results = list(executor.map(_kalman_fallback, tasks))
                            results.sort(key=lambda x: x[0])
                            preds = [r[1] for r in results]
                            break
                        continue
